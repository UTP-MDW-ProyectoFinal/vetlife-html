(function () {
  ...
  function protegerPaginas() {
    ...
    if (paginaRequerida === 'usuario' && (!sesion || sesion.rol !== 'usuario')) {
      window.location.replace(`login.html?redirect=${encodeURIComponent(window.location.pathname.split('/').pop())}`);
      return;
    }

    if (paginaRequerida === 'administrador' && (!sesion || sesion.rol !== 'administrador')) {
      window.location.replace('../login.html?redirect=admin');
    }
  }

  function configurarMenuMovil() {
    const boton = document.querySelector('.navbar-toggler[data-bs-target="#menuPrincipal"]');
    const menu = document.getElementById('menuPrincipal');
    if (!boton || !menu) return;

    // El navbar responsive utiliza el componente Collapse oficial de Bootstrap.
    // Si Bootstrap no carga, mantenemos un respaldo mínimo para que la navegación
    // siga siendo accesible.
    if (window.bootstrap?.Collapse) {
      window.bootstrap.Collapse.getOrCreateInstance(menu, { toggle: false });
      return;
    }
  ...
  function configurarContacto() {
    ...
    mapa.src = 'https://www.openstreetmap.org/export/embed.html?bbox=-80.645%2C-5.205%2C-80.620%2C-5.185&layer=mapnik&marker=-5.19449%2C-80.63282';
  }
