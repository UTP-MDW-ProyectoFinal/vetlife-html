(function () {
    ...
    contenedor.innerHTML = categorias.map(categoria => `
      <section class="mb-5">
        <div class="d-flex align-items-center gap-2 mb-3">
          <span class="badge bg-principal-suave text-principal">${escaparHtml(categoria)}</span>
        ...
        <div class="row row-cols-1 row-cols-md-2 row-cols-xl-3 g-4">
          ${servicios.filter(servicio => servicio.categoria === categoria).map(servicio => `
            <article class="col">
              <div class="card tarjeta-vet h-100">
                <div class="card-body p-4">
                  <div class="icono-servicio mb-3"><i class="bi ${escaparHtml(servicio.icono)}"></i></div>
                  ...
                  <div class="d-flex justify-content-between small">
                    ...
                    <strong class="text-principal">S/ ${servicio.precio}</strong>
                  </div>
                </div>
              </div>
            </article>`).join('')}
  ...
  function configurarInicio() {
    ...
    if (contenedorPasos) {
      contenedorPasos.innerHTML = VetLifeDatos.pasos.map(paso => `
        <div class="col-md-4">
          <div class="card tarjeta-vet h-100">
            <div class="card-body p-4">
              <span class="badge bg-principal-suave text-principal mb-3">${escaparHtml(paso.numero)}</span>
